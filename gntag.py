...
@app.on_message(filters.command("gn") & filters.group & admin_filter)
async def goodnight_tag(_, message):
    try:
        ...
        async for member in app.get_chat_members(message.chat.id):
            if not member.user.is_bot:  # Skip bots
                ...
                if usernum == 5:
                    goodnight_msg = f"🌙 **Good Night Everyone**\n\n{text}\n\n{usertxt}"
                    await app.send_message(message.chat.id, goodnight_msg)
